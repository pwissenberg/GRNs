...
src_dirs = /usr/local/data_pipeline:/opt/data_pipeline
...
if sys.platform == 'win32':
    ...
    default_src_dirs = ['.', '/usr/local/data_pipeline', '/opt/data_pipeline', '/sw/data_pipeline']
...
if os.path.join(sys.prefix, 'lib') not in default_lib_dirs:
    ...
    default_src_dirs.append(os.path.join(sys.prefix, 'data_pipeline'))
...
class f2py_info(system_info):
    def calc_info(self):
        try:
            ...
            return
        f2py_dir = os.path.join(os.path.dirname(f2py.__file__), 'data_pipeline')
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'libs', 'python', 'data_pipeline',
                                           'module.cpp')):
                src_dir = d
                break
        if not src_dir:
        ...
        srcs_dir = os.path.join(src_dir, 'libs', 'python', 'data_pipeline')
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'data_pipeline', 'agg_affine_matrix.cpp')):
                src_dir = d
                break
        if not src_dir:
            return
        if sys.platform == 'win32':
            agg2_srcs = glob(os.path.join(src_dir, 'data_pipeline', 'platform',
                                          'win32', 'agg_win32_bmp.cpp'))
        else:
            agg2_srcs = glob(os.path.join(src_dir, 'data_pipeline', '*.cpp'))
            agg2_srcs += [os.path.join(src_dir, 'data_pipeline', 'platform',
                                       'X11',
