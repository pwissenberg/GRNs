...
- "data_pipeline-layout": a directory representing a Python project that contains a "data_pipeline"
  folder. Everything under the "data_pipeline" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "data_pipeline-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "data_pipeline-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``data_pipeline`` directory
        (or anything pointed by ``package_dir[""]``).

        The "data_pipeline-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``data_pipeline`` directory exists,
        this function will set ``package_dir[""] = "data_pipeline"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "data_pipeline"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`data_pipeline-layout` detected -- analysing {src_dir}")
